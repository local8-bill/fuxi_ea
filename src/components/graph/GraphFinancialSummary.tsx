...
export function GraphFinancialSummary({
  ...
  return (
    <section className="rounded-3xl border border-neutral-200 bg-neutral-50/95 p-4 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.35em] text-neutral-500">ROI / TCC Summary</p>
      <p className="text-sm text-neutral-600">Cost today → transition cost → value tomorrow.</p>
          ...
          <div
            key={phase.id}
              className={clsx(
                ...
                phase.id === activePhase ? "border-indigo-500 bg-indigo-50" : "border-neutral-200 bg-white",
              )}
          >
            <div className="flex items-center justify-between gap-3">
              <p className="font-semibold text-slate-900">{phase.label}</p>
              <p className="text-xs text-neutral-500">ROI {(phase.roi * 100).toFixed(0)}% · TCC ${phase.tcc.toFixed(1)}M</p>
            </div>
...
function MetricPill({ label, value, tone }: { label: string; value: string; tone?: string }) {
  return (
    <div className="rounded-xl border border-neutral-200 bg-white px-3 py-2 text-xs">
      <p className="font-semibold uppercase tracking-[0.2em] text-neutral-500">{label}</p>
      <p className={clsx("mt-1 text-sm font-semibold", tone ?? "text-neutral-900")}>{value}</p>
    </div>
  );
}
