  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white/95 p-4 shadow-sm">
      <div className="flex items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.35em] text-slate-500">Decision Backlog</p>
          <p className="text-sm text-slate-600">Stakeholder influence + ROI/TCC weighting</p>
        </div>
      </div>
      <ul className="mt-3 space-y-2 text-sm text-slate-700">
        {nodes.map((node) => {
          ...
          return (
            <li
              key={node.id}
              className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm hover:border-slate-900 cursor-pointer"
            ...
            >
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-[0.65rem] text-slate-500">
