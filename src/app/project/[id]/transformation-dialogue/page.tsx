...
function buildItems(graph: HarmonizedGraph): TransformationItem[] {
  const nodes = graph.nodes ?? [];
  // Focus on delta nodes (added/removed/modified) as transformation candidates.
  const candidates = nodes.filter((n) => n.state !== "unchanged");
...
export default async function TransformationDialoguePage({ params }: PageProps): Promise<ReactElement> {
  ...
  if (!graph || !graph.nodes?.length) {
    return (
      <div className="mx-auto max-w-5xl px-6 py-12">
        <h1 className="text-2xl font-semibold text-slate-900">Transformation Dialogue</h1>
        <p className="mt-3 text-slate-600">
          No harmonized data is available yet. Complete harmonization first, then return to define transformation actions.
        </p>
      </div>
    );
  }
  ...
  return <TransformationDialogueClient projectId={projectId} items={items} />;
}
